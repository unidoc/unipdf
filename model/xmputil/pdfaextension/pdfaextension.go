...
package pdfaextension ;import (_b "fmt";_a "github.com/trimmer-io/go-xmp/models/dc";_ac "github.com/trimmer-io/go-xmp/models/pdf";_ge "github.com/trimmer-io/go-xmp/models/xmp_base";_gf "github.com/trimmer-io/go-xmp/models/xmp_mm";_cc "github.com/trimmer-io/go-xmp/xmp";
...
func init (){_cc .Register (Namespace ,_cc .XmpMetadata );_cc .Register (SchemaNS );_cc .Register (PropertyNS );_cc .Register (ValueTypeNS );_cc .Register (FieldNS );};
...
func FillModel (d *_cc .Document ,extModel *Model )error {var _af []*_cc .Namespace ;for _ ,_bb :=range d .Namespaces (){_af =append (_af ,_bb );};_c .Slice (_af ,func (_gd ,_dd int )bool {return _af [_gd ].Name < _af [_dd ].Name });for _ ,_acd :=range _af {switch _acd {case Namespace ,SchemaNS ,PropertyNS ,ValueTypeNS ,FieldNS ,_ge .NsXmp ,_a .NsDc :continue ;
default:_ff ,_agd :=GetSchema (_acd .URI );if !_agd {continue ;};_db :=d .FindModel (_acd );_ba :=*_ff ;_ba .Property =Properties {};for _ ,_fgc :=range _ff .Property {_ccf :=_cb (_db ,_acd ,_fgc );if _ccf {_ba .Property =append (_ba .Property ,_fgc );
};};if len (_ba .Property )==0{continue ;};var _ffe bool ;for _dgc ,_bef :=range extModel .Schemas {if _bef .Schema ==_ba .Schema {_ffe =true ;extModel .Schemas [_dgc ]=_ba ;break ;};};if !_ffe {extModel .Schemas =append (extModel .Schemas ,_ba );};};};
return nil ;};

// Typ gets the array type of properties.
func (_eg Properties )Typ ()_cc .ArrayType {return _cc .ArrayTypeOrdered };const (ValueTypeResourceEvent ValueTypeName ="\u0052\u0065\u0073\u006f\u0075\u0072\u0063\u0065\u0045\u0076\u0065\u006e\u0074";);
...
func init (){_ae =map[string ]*Schema {_gf .NsXmpMM .URI :&XmpMediaManagementSchema ,"\u0078\u006d\u0070\u0069\u0064\u0071":&XmpIDQualSchema ,_ac .NsPDF .URI :&PdfSchema ,"\u0073\u0074\u0045v\u0074":&FieldResourceEventSchema ,"\u0073\u0074\u0056e\u0072":&FieldVersionSchema };
};
...
func (_cd ValueTypes )MarshalXMP (e *_cc .Encoder ,node *_cc .Node ,m _cc .Model )error {return _cc .MarshalArray (e ,node ,_cd .Typ (),_cd );};

// GetSchema for provided namespace.
func GetSchema (namespaceURI string )(*Schema ,bool ){_dgb .RLock ();defer _dgb .RUnlock ();_bf ,_cbb :=_ae [namespaceURI ];return _bf ,_cbb ;};
...
case PropertyCategoryUndefined :return []byte (""),nil ;default:return nil ,_b .Errorf ("\u0075\u006ed\u0065\u0066\u0069\u006ee\u0064\u0020p\u0072\u006f\u0070\u0065\u0072\u0074\u0079\u0020c\u0061\u0074\u0065\u0067\u006f\u0072\u0079\u0020\u0076\u0061\u006c\u0075e\u003a\u0020\u0025\u0076",_fc );
};};

// MakeModel creates or gets a model from document.
func MakeModel (d *_cc .Document )(*Model ,error ){_be ,_fg :=d .MakeModel (Namespace );if _fg !=nil {return nil ,_fg ;};return _be .(*Model ),nil ;};const (PropertyCategoryUndefined PropertyCategory =iota ;PropertyCategoryInternal ;PropertyCategoryExternal ;
...
func RegisterSchema (ns *_cc .Namespace ,schema *Schema ){_dgb .Lock ();defer _dgb .Unlock ();_ae [ns .URI ]=schema ;};func _cb (_ddf _cc .Model ,_fff *_cc .Namespace ,_ed Property )bool {_ec :=_g .ValueOf (_ddf ).Elem ();if _ec .Kind ()==_g .Ptr {_ec =_ec .Elem ();
};_df :=_ec .Type ();for _bed :=0;_bed < _df .NumField ();_bed ++{_ab :=_df .Field (_bed );_da :=_ab .Tag .Get ("\u0078\u006d\u0070");if _da ==""{continue ;};if !_dg .HasPrefix (_da ,_fff .Name ){continue ;};_fa :=_dg .IndexRune (_da ,':');if _fa ==-1{continue ;
};_fe :=_da [_fa +1:];if _fe ==_ed .Name {_daf :=_ec .Field (_bed );return !_daf .IsZero ();};};return false ;};
...
// MarshalXMP implements xmp.Marshaler interface.
func (_dfb Properties )MarshalXMP (e *_cc .Encoder ,node *_cc .Node ,m _cc .Model )error {return _cc .MarshalArray (e ,node ,_dfb .Typ (),_dfb );};

// UnmarshalXMP implements xmp.Unmarshaler interface.
func (_gg *Schemas )UnmarshalXMP (d *_cc .Decoder ,node *_cc .Node ,m _cc .Model )error {return _cc .UnmarshalArray (d ,node ,_gg .Typ (),_gg );};func (_ccfg Schemas )Typ ()_cc .ArrayType {return _cc .ArrayTypeUnordered };
...
func (_bd *Model )SetTag (tag ,value string )error {if _ce :=_cc .SetNativeField (_bd ,tag ,value );_ce !=nil {return _b .Errorf ("\u0025\u0073\u003a\u0020\u0025\u0076",Namespace .GetName (),_ce );};return nil ;};

// ValueTypes is the slice of field value types.
type ValueTypes []FieldValueType ;

// CanTag implements xmp.Model interface.
func (_daa *Model )CanTag (tag string )bool {_ ,_ea :=_cc .GetNativeField (_daa ,tag );return _ea ==nil ;};
...
// MarshalXMP implements xmp.Marshaler interface.
func (_edb Schemas )MarshalXMP (e *_cc .Encoder ,node *_cc .Node ,m _cc .Model )error {return _cc .MarshalArray (e ,node ,_edb .Typ (),_edb );};
...
func (_baf *Model )GetTag (tag string )(string ,error ){_dfa ,_eab :=_cc .GetNativeField (_baf ,tag );if _eab !=nil {return "",_b .Errorf ("\u0025\u0073\u003a\u0020\u0025\u0076",Namespace .GetName (),_eab );};return _dfa ,nil ;};var (Namespace =_cc .NewNamespace ("\u0070\u0064\u0066\u0061\u0045\u0078\u0074\u0065\u006e\u0073\u0069\u006f\u006e","\u0068\u0074\u0074\u0070\u003a\u002f\u002f\u0077\u0077\u0077\u002e\u0061\u0069\u0069\u006d\u002e\u006f\u0072\u0067\u002f\u0070\u0064\u0066\u0061/\u006e\u0073\u002f\u0065\u0078t\u0065\u006es\u0069\u006f\u006e\u002f",NewModel );
...
func (_ccc *Model )SetSchema (namespaceURI string ,s Schema ){for _gfg :=0;_gfg < len (_ccc .Schemas );_gfg ++{if _ccc .Schemas [_gfg ].NamespaceURI ==namespaceURI {_ccc .Schemas [_gfg ]=s ;return ;};};_ccc .Schemas =append (_ccc .Schemas ,s );};
