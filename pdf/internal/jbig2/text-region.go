...
func newTextRegion(r reader.StreamReader, h *SegmentHeader) *TextRegion {
	t := &TextRegion{
		r:             r,
		segmentHeader: h,
...
func (t *TextRegion) Init(header *SegmentHeader, r reader.StreamReader) error {
	t.segmentHeader = header
	t.r = r
...
func (t *TextRegion) getSymbols() error {
	if t.segmentHeader.RTSegments != nil {
...
func (t *TextRegion) initSymbols() error {
	for _, segment := range t.segmentHeader.RTSegments {
		if segment.SegmentType == 0 {
			s, err := segment.getSegmentData()
			if err != nil {
				return err
			}
			sd, ok := s.(*SymbolDictionarySegment)
			if !ok {
...
func (t *TextRegion) getUserTable(tablePosition int) (huffman.HuffmanTabler, error) {
	...
	for _, rts := range t.segmentHeader.RTSegments {
		if rts.SegmentType == 53 {
