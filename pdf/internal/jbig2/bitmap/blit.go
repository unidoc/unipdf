...
func Blit(src *Bitmap, dst *Bitmap, x, y int, op CombinationOperator) (err error) {
	...
	// common.Log.Trace("Blitting bitmaps with operator: %s, at X: %d, Y: %d", op, x, y)
	// common.Log.Trace("Source bitmap: %s", src)
	// common.Log.Trace("Dst bitmap: %s", dst)
	...
	// common.Log.Trace("Blit params - shiftVal1: %v, shiftVal2: %v, padding: %v, toShift: %v, useShift: %v, dstStartIdx: %v", shiftVal1, shiftVal2, padding, toShift, useShift, dstStartIdx)
	...
	if err != nil {
		common.Log.Trace("Blit failed: %s", err)
	}
...
func blitUnshifted(
	...
	// common.Log.Trace("Blit unshifted")
	var dstLine int
...
func blitSpecialShifted(
	...
	// common.Log.Trace("Blit special shifted")
	var dstLine int
	increaser := func() {
		dstLine++
		dstStartIdx += dst.RowStride
